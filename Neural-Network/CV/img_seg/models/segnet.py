...
from utils.torch_utils import set_trainable


class SegNet(nn.Module):
    def __init__(self, num_classes, in_channels=3, freeze_bn=False, freeze_backbone=False, pretrained=True):
        ...
        self.stage1_encoder = nn.Sequential(*encoder[:6])  # [conv bn act]*2
        self.stage2_encoder = nn.Sequential(*encoder[7:13])  #  [conv bn act]*2
        self.stage3_encoder = nn.Sequential(*encoder[14:23])  #  [conv bn act]*3
        self.stage4_encoder = nn.Sequential(*encoder[24:33])  #  [conv bn act]*3
        self.stage5_encoder = nn.Sequential(*encoder[34:-1])  #  [conv bn act]*3
...
class SegResNet(nn.Module):
    def __init__(self, num_classes, in_channels=3, freeze_bn=False, pretrained=True):
        ...
        if in_channels != 3:
            encoder[0] = nn.Conv2d(in_channels, 64, kernel_size=3, stride=1, padding=1)
        encoder[3].return_indices = True  # pooling
