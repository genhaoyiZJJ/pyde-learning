...
from configs.config_cifar import cfg
...
if __name__ == "__main__":
    ...
    train_dataset = CifarLTDataset(img_dir=cfg.train_dir, transform=cfg.train_transform, is_train=True)
    
    # valid
    valid_dataset = CifarLTDataset(img_dir=cfg.valid_dir, transform=cfg.valid_transform, is_train=False)
    ...
    if cfg.pb_sampling:
        ...
        progressive_sampler = ProgressiveSampler(train_dataset, cfg.max_epoch)
    ...
    if cfg.label_smoothing:
        from utils.label_smoothing_ce import LabelSmoothingCE
        loss_fn = LabelSmoothingCE(cfg.smoothing_factor)
    else:
    ...
    for epoch in range(cfg.max_epoch):
        if cfg.pb_sampling:
            sampler, _ = progressive_sampler(epoch)
