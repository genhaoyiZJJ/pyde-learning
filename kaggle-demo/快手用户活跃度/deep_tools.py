...
class DataGenerator:
    ...
    def get_set(self, usage='train'):
        ...
        data_list = [np.array([self.data[user_id] for user_id in self.user_queue[seq_length]]) for seq_length in
                     test_list]
        label_list = [np.array([self.label[user_id] for user_id in self.user_queue[seq_length]]) for seq_length in
                      test_list]
